  ...
  for (int i = 0; i < 4; i++)
    if (ide_devices[i].Reserved == 1) {
      printk(" %d Found %s Drive %dMB - %s\n", i,
             ...
             ide_devices[i].Model);
      strcpy(vd.drive_name, (cstr)ide_devices[i].Model);
...
u8 ide_print_error(u32 drive, u8 err) {
  ...
  if (err == 1) {
    ...
    if (st & ATA_ER_AMNF) {
      klogf("- No Address Mark Found\n     ");
      err = 7;
    }
    if (st & ATA_ER_TK0NF) {
      klogf("- No Media or Media Error\n     ");
      err = 3;
    }
    if (st & ATA_ER_ABRT) {
      klogf("- Command Aborted\n     ");
      err = 20;
    }
    if (st & ATA_ER_MCR) {
      klogf("- No Media or Media Error\n     ");
    ...
    if (st & ATA_ER_UNC) {
      klogf("- Uncorrectable Data Error\n     ");
      err = 22;
    }
    if (st & ATA_ER_BBK) {
      klogf("- Bad Sectors\n     ");
