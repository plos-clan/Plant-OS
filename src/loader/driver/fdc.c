...
void floppy_init() {
  ...
  if (getbyte() == -1) {
    klogf("floppy: no floppy drive found");
    klogf("No fount FDC");
    return;
  }
  //设置软盘驱动器的中断服务程序
...
int fdc_rw(int block, u8 *blockbuff, int read, u32 nosectors) {
  ...
  for (tries = 0; tries < 3; tries++) {
    ...
    /* 发送命令 */
    if (read) {
      dma8_send(2, (void *)tbaddr, nosectors * 512, 0);
      sendbyte(CMD_READ);
    } else {
      dma8_send(2, (void *)tbaddr, nosectors * 512, 1);
      sendbyte(CMD_WRITE);
    }
