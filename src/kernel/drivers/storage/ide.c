...
static u8 ide_read(u8 channel, u8 reg) {
  ...
  if (reg < 0x08)
    result = asm_in8(channels[channel].base + reg - 0x00);
  else if (reg < 0x0C)
    result = asm_in8(channels[channel].base + reg - 0x06);
  else if (reg < 0x0E)
    result = asm_in8(channels[channel].ctrl + reg - 0x0A);
  else if (reg < 0x16)
    result = asm_in8(channels[channel].bmide + reg - 0x0E);
...
static void ide_write(u8 channel, u8 reg, u8 data) {
  if (reg > 0x07 && reg < 0x0C) ide_write(channel, ATA_REG_CONTROL, 0x80 | channels[channel].nIEN);
  if (reg < 0x08)
    asm_out8(channels[channel].base + reg - 0x00, data);
  else if (reg < 0x0C)
    asm_out8(channels[channel].base + reg - 0x06, data);
  else if (reg < 0x0E)
    asm_out8(channels[channel].ctrl + reg - 0x0A, data);
  else if (reg < 0x16)
    asm_out8(channels[channel].bmide + reg - 0x0E, data);
  if (reg > 0x07 && reg < 0x0C) ide_write(channel, ATA_REG_CONTROL, channels[channel].nIEN);
}
void ide_read_buffer(u8 channel, u8 reg, u32 buffer, u32 quads) {
...
FASTCALL void ide_irq(i32 id, regs32 *regs) {
  klog("ide irq.");
  ide_irq_invoked = 1;
}
