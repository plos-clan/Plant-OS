...
dlimport int printf(const char *_rest fmt, ...) __THROWNL __attr(format(printf, 1, 2));

//* ----------------------------------------------------------------------------------------------------
//& 内存分配
// 为了高效地访问内存，建议 8 或 16 字节对齐
dlimport void *malloc(size_t size) __THROW __attr_malloc __attr_allocsize(1) __wur;
